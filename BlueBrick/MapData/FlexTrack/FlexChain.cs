...
namespace BlueBrick.MapData.FlexTrack
{
	...
	{
		// data members
		List<IKSolver.Bone_2D_CCD> mBoneList;
		...
		public static FlexChain createFlexChain(List<LayerBrick.Brick> trackList, LayerBrick.Brick grabbedTrack)
		{
			...
			// the chain to return
			FlexChain flexChain = new FlexChain();
			...
			while ((currentTrack != null) && (currentTrack.ConnectionPoints.Count == 2))
			{
				...
				newBone.x = (currentConnection.PositionInStudWorldCoord.X - nextConnection.PositionInStudWorldCoord.X);
				newBone.y = (currentConnection.PositionInStudWorldCoord.Y - nextConnection.PositionInStudWorldCoord.Y);
				newBone.angle = 0;
				flexChain.mBoneList.Add(newBone);

				// advance to the next link
				currentTrack = nextConnection.ConnectedBrick;
